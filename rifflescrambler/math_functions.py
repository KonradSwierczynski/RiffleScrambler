...
def riffle_permutation(B):
    ...
    riffle_permutation_func = lambda i: rank(b, i) if b[i] == '0' else rank(b, i) + b.count('0')
    return [riffle_permutation_func(i) for i in range(8)]


def inverted_riffle_permutation(B, searched_value):
    perm = riffle_permutation(B)
    return inverted_perm(perm, searched_value)


def inverted_perm(perm, searched_value):
...
def trace_trajectories(B):
    ...
    for i in range(1, len(B)):
        ...
        trajectories.append(inverted_riffle_permutation(B[i - 1], B[i]))
    return trajectories
