...
\begin{definition}
	(\textit{Równoległe/Sekwencyjne Etykietowanie Grafu}). Niech $G = (V, E)$ będzie grafem skierowanym grafem acyklicznym i niech $T \subset V$ będzie zbiorem wierzchołków do oetykietowania. $T$ będzie nazywane celem.
...
\begin{lemma}
	\cite[Lemat 1]{rs}(Ograniczenie dolne dla (N, $ \lambda $)-Superkoncentratora).
...
\begin{theorem} \cite[Twierdzenie 10]{depth}
...
\begin{definition}
	(Graf N rzędowy). Niech $n, N \in \mathbb{N}^{+}$ takie, że $N + 1$ dzieli $n$ oraz niech $k=n/(N + 1)$. Mówimy, że graf $G$ jest $N$ rzędowy, jeżeli $G$ zawiera ścieżkę przechodzącą przez $n$ wierzchołków $(v_{1},\dots,v_{n})$ oraz dzieląc go na warstwy $L_{j} = \{v_{jk + 1},\dots,v_{jk + k} \}$, dla $j \in \{0,\dots,N\}$, pozostałe krawędzie łączą wierzchołki z niższej warstwy $L_{j}$ jedynie z wierzchołkami z wyższych warstw $L_{i}$, $i \in \{ j+1,\dots,N \}$.
\end{definition}



\begin{lemma} \cite[Lemat 4.2]{alwen2016efficiently}
	Niech $G$ będzie $N$ rzędowym grafem, wtedy dla dowolnego $t \in \mathbb{N}^{+}$, $G$ jest $(n/t, N t)$-reducible.
\end{lemma}
...
\begin{proof}
	...
	Pozostaje obliczyć górne ograniczenie używając \cite[Twierdzenie 6]{rs}.
...
\begin{proof}
	...
	Z \cite[Lemat 4.2]{alwen2016efficiently} wynika więc, że $RSG_{\lambda}^{N}$  jest ($n / t$, $\lambda + t - \lambda - 1$)-reducible dla dowolnego $t \geq 1$.
	Z twierdzenia \cite[Twierdzenie 10]{depth} wynika, że $\Pi_{cc}^{ \parallel }(RSG_{\lambda}^{\overline{n}}) = O \left( n \left( \sqrt{( \lambda t + t - \lambda - 1)n \delta } + \frac{n}{t} \right) \right)$.
	...
	Zatem podstawiając $t$ minimalizujące ograniczenie górne do wzoru z \cite[Twierdzenie 10]{depth} otrzymujemy
